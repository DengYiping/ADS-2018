object HeapSort {
  private def indexLeft(i: Int) = (2 * i) + 1 // zero indexed system
  private def indexRight(i: Int) = indexLeft(i) + 1
  private def indexParent(i: Int) = (i - 1) / 2
  ...
  def maxHeapify[T](a: Array[T], i: Int, size: Int)(implicit ord: Ordering[T]): Unit = {
    ...
    val largest =
      if(idxLeft < size && ord.gt(a(idxLeft), a(i)))
        idxLeft
      else if(idxRight < size && ord.gt(a(idxRight), a(i)))
        idxRight
      else
        i
    if(largest != i){
